...
def test_threadSets(sbh_connector, thread_size, sbolFile, sbolDoc_size, collPrefix):
	...
	for threadSet_val in range(1, thread_size+1):
		...
		for t in thread_set:
			thread_id = "set" + str(threadSet_val) + "_thread" + str(t)
			total_threadTime += t.thread_duration()
			thread_duration[thread_id]= t.thread_duration()
		# setTime_List.append(total_threadTime)

	df = pd.DataFrame.from_dict(thread_duration, orient='columns')
	# df = pd.DataFrame(thread_duration, index=[0])
		
	fig, ax = plt.subplots()
	ax.set_title("Time Taken to Push %s SBOL Documents for each set of Threads" %sbolDoc_size)
	ax.set_ylabel("Total Push Time per Set (sec)")
	ax.set_xlabel("# of Sets Executed for %s of Threads Ran per Set" %sbolDoc_size)
